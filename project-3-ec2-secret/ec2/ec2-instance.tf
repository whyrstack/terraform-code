locals {
    virtual_machines = {
        ...
        "vm2" = { ami = data.aws_ami.amzlinux2.id, instance_type = "t2.medium", availability_zone = data.aws_availability_zones.available.names[0], instance_count = 1, subnet_ids = data.aws_subnets.subnets_public.ids[0], vpc_security_group_ids = data.aws_security_groups.selected.ids }
    }
}
